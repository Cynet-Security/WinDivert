...
/*
 * DESCRIPTION:
 * This is a simple traffic monitor.  It uses a CyDivert handle in SNIFF mode.
 * The SNIFF mode copies packets and does not block the original.
 *
 * usage: netdump.exe cydivert-filter [priority]
 *
 */
...
int __cdecl main(int argc, char **argv)
{
    ...
    handle = CyDivertOpen(argv[1], CYDIVERT_LAYER_NETWORK, priority,
        CYDIVERT_FLAG_SNIFF | CYDIVERT_FLAG_FRAGMENTS);
    ...
    if (!CyDivertSetParam(handle, CYDIVERT_PARAM_QUEUE_TIME,
    ...
    if (!CyDivertSetParam(handle, CYDIVERT_PARAM_QUEUE_SIZE,
    ...
    while (TRUE)
    {
        ...
        hash = CyDivertHelperHashPacket(packet, packet_len, 0);
        ...
        if (ip_header != NULL)
        {
            ...
            printf("IPv4 [Version=%u HdrLength=%u TOS=%u Length=%u Id=0x%.4X "
                ...
                ntohs(ip_header->Id), CYDIVERT_IPHDR_GET_RESERVED(ip_header),
                CYDIVERT_IPHDR_GET_DF(ip_header),
                CYDIVERT_IPHDR_GET_MF(ip_header),
                ntohs(CYDIVERT_IPHDR_GET_FRAGOFF(ip_header)), ip_header->TTL,
        ...
        if (ipv6_header != NULL)
        {
            ...
            printf("IPv6 [Version=%u TrafficClass=%u FlowLabel=%u Length=%u "
                ...
                ipv6_header->Version,
                CYDIVERT_IPV6HDR_GET_TRAFFICCLASS(ipv6_header),
                ntohl(CYDIVERT_IPV6HDR_GET_FLOWLABEL(ipv6_header)),
